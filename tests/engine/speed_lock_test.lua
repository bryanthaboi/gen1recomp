...
do
  ...
  eq(g:logicSpeed(), 1, "gen1 battle is 1X at speedBattle 200")
  g.speedOverride = 200
  eq(g:logicSpeed(), 1, "gen1 battle ignores speedOverride (--speed, skin FF hold)")
  local bus = Hooks.new()
  ...
  g.speedOverride = nil
  eq(g:logicSpeed(), 1, "gen1 battle ignores a core.logic_speed hook returning 200")
...
do
  ...
  g:_cycleSpeed(-1)
  g:_cycleSpeed(1)
  eq(g.save.options.speedBattle, 1, "gen1 speed presses in battle leave speedBattle")
  eq(g.save.options.speedOverworld, 1, "and speedOverworld")
  g:touchSkinHotkey("fast_forward_toggle", true)
  eq(g.save.options.speedBattle, 1, "gen1 skin FF toggle in battle is ignored")
  g:touchSkinHotkey("fast_forward_hold", true)
  eq(g:logicSpeed(), 1, "gen1 skin FF hold in battle stays 1X")
...
do
  ...
  eq(gen2({ options = { speed = 200 }, stack = stack({}, battle) }):logicSpeed(), 1,
    "gen2 battle is 1X")
  eq(gen2({ options = { speed = 200 }, speedOverride = 200,
            stack = stack({}, battle) }):logicSpeed(), 1,
    "gen2 battle ignores speedOverride")
  ...
  g:_cycleSpeed(1)
  eq(g.options.speed, 1, "gen2 pad speed press in battle is ignored")
  g.stack = stack({})
  g.linkNet = { closed = false }
  g:_cycleSpeed(1)
  eq(g.options.speed, 1, "gen2 pad speed press over a link is ignored")
end
...
do
  ...
  battleActive = true
  eq(g:logicSpeed(), 1, "gen3 battle is 1X")
  g.speedOverride = 200
  eq(g:logicSpeed(), 1, "gen3 battle ignores speedOverride")
  g.speedOverride = nil
  g:_cycleSpeed(1)
  eq(g.options.speedBattle, 200, "gen3 speed press in battle is ignored")
  battleActive = false
...
local function runFrame(g, lockAt)
  ...
  FixedStep:init(function()
    steps = steps + 1
    if steps == lockAt then battleActive = true; g.stack = stack(overworld, battle) end
    g:_speedLockEdge()
  end)
...
do
  ...
  eq(steps, 3, "gen3: a battle starting mid-frame ends the frame on that step")
  battleActive = false

  local g1 = gen1({ overworld })
  steps = runFrame(g1, 5)
  eq(steps, 5, "gen1: a battle push mid-frame ends the frame on that step")
  eq(g1:logicSpeed(), 1, "and the next frame samples 1X")

  local g2 = gen2({ options = { speed = 200 }, stack = stack({}) })
  steps = runFrame(g2, 2)
  eq(steps, 2, "gen2: a battle push mid-frame ends the frame on that step")
  battleActive = false
